...
export interface Actor{id:FighterId;x:number;y:number;vy:number;face:1|-1;hp:number;action:Action;age:number;lock:number;specialCd:number;attackHit:boolean;buffer:Token[];last:Input;wins:number;combo:number;lastHit:number;activeMove:number;chargeTicks:number;chargeGrace:number;chargeFace:number;queued:{key:"punch"|"kick"|"jump"|"ultra";tick:number}|null;parryTicks:number;parryCd:number;invuln:number}
...
const actor=(id:FighterId,x:number,face:1|-1):Actor=>({id,x,y:0,vy:0,face,hp:100,action:'idle',age:0,lock:0,specialCd:0,attackHit:false,buffer:[],last:neutral(),wins:0,combo:0,lastHit:-999,activeMove:0,chargeTicks:0,chargeGrace:0,chargeFace:face,queued:null,parryTicks:0,parryCd:0,invuln:0});
...
function commands(m:Match,index:number,input:Input){
 ...
 else if((queued==='jump'||input.jump&&!a.last.jump)&&a.y===0){a.vy=-625;setAction(a,'jump');m.events.push({type:'jump',player:index})}
...
export function step(m:Match,inputs:[Input,Input]){
 ...
 m.actors.forEach((a,index)=>{
  ...
  if((a.action==='punch'||a.action==='kick')&&!a.attackHit){if(meleeConnects(a,other)){a.attackHit=true;damage(m,index,1-index,a.action==='punch'?8:12,other.x,FLOOR+other.y-65)}}
