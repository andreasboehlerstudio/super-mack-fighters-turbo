...
export function loadCostumeSheet(id:FighterId,costume:Costume,kind:'walk'|'action'|'motion'='action'):Promise<HTMLCanvasElement>{
 ...
 const promise=(async()=>{
  const base=await load(`/assets/${kind==='action'?'':kind+'/'}${id}.png`),out=document.createElement('canvas');
  ...
  if(kind==='motion')motionAnchors??=fetch('/assets/motion/anchors.json').then(r=>{if(!r.ok)throw Error('Motion costume anchors unavailable');return r.json()});
