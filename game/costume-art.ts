...
export function loadCostumeSheet(id:FighterId,costume:Costume,kind:'walk'|'action'='action'):Promise<HTMLCanvasElement>{
 const key=`${id}-${costume}-${kind}`;if(cache.has(key))return cache.get(key)!;
 const promise=(async()=>{
  const base=await load(`/assets/${kind==='walk'?'walk/':''}${id}.png`),out=document.createElement('canvas');
  ...
  anchors??=fetch('/assets/costume-anchors.json').then(r=>{if(!r.ok)throw Error('Costume anchors unavailable');return r.json()});
  ...
  for(let y=0;y<out.height;y+=256)for(let x=0;x<out.width;x+=256){
   const anchor=poses[id]?.[kind]?.[frame++];if(!anchor)throw Error(`Missing costume pose ${id}/${kind}/${frame-1}`);
