...
export function MusicJukebox({audio,initialArea}:{audio:ArcadeAudio|null;initialArea:string}){
 const [index,setIndex]=useState(Math.max(0,AREA_THEMES.findIndex(t=>t.id===initialArea))),[playing,setPlaying]=useState(false),[error,setError]=useState('');
 ...
 const play=async(id:string)=>{if(!audio)return;const ticket=++request.current;try{await audio.unlock();if(ticket!==request.current)return;audio.previewArea(id);active.current=true;setPlaying(true);setError('')}catch{setError('Sound konnte nicht gestartet werden. Bitte noch einmal starten.')}};
 const stop=()=>{request.current++;audio?.previewArea(null);active.current=false;setPlaying(false)};
