...
test('Karsten, Edda and Michael Scholz have themed move sets and fit the portrait grid',()=>{
 ...
 assert.equal(SELECT_PAGE_SIZE,SELECT_COLUMNS*SELECT_ROWS);assert.equal(Math.ceil(HEROES.length/SELECT_PAGE_SIZE),2);
});

test('new fighters execute all three specials and retain projectile identity after a tag',()=>{
 for(const id of ['schaer','glen','steffen','vendel','karsten','edda','scholz','bobo','mross','otto','ross','freudenreich','robbemond','olli','boeckli','louis','tesla'] as FighterId[])for(let move=0;move<3;move++){
