...
export function TourMap({route,index,compact=false,position}:{route:string[];index:number;compact?:boolean;position?:Point|null}){
 ...
 return <section className={`tour-map ${compact?'compact-map':''}`} aria-label="Parkplan deiner Arena-Tour">
  ...
  <div className="park-map-surface">
   ...
   {!point&&<div className="map-chapter"><span>{inspected==='finale'?'★ FINALE':'◆ HINTER DEN KULISSEN'}</span><p>Deine Tour führt jetzt in die MACK-Welt.</p></div>}
  ...
  <div className="backstage-route"><div className="backstage-heading">HINTER DEN KULISSEN <span>Eigenes Tourkapitel außerhalb des Parkplans</span></div><div className="backstage-nodes">{backstage.map(id=>{const s=station(id),state=mapStatus(id,route,index);return <button key={id} className={`backstage-node ${state} ${id===inspected?'inspected':''}`} onClick={()=>setInspected(id)} aria-label={`${s.name} auf der Tour`}><i>{state==='done'?'✓':id==='finale'?'★':route.indexOf(id)+1}</i><span>{s.name}</span></button>})}</div></div>
