...
test('all tours open with Frederik, meet Michael at midpoint and end with Roland',()=>{
 ...
 assert.deepEqual(new Set(arcadeLadder(21)),new Set(HEROES.map(f=>f.id)));assert.deepEqual(arcadeLadder(1),['roland']);assert.deepEqual(arcadeLadder(2),['frederik','roland']);
});
