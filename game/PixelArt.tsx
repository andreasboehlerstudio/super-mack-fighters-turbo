...
export function PixelArt({src,alt,className='',density=1,onLoad,onError}:{src:string;alt:string;className?:string;density?:number;onLoad?:()=>void;onError?:()=>void}){
 ...
 useEffect(()=>{const host=ref.current!,canvas=host.querySelector('canvas')!,ctx=canvas.getContext('2d')!,img=new Image();let alive=true;
  ...
  img.onload=()=>{draw();callbacks.current.onLoad?.()};img.onerror=()=>callbacks.current.onError?.();img.src=src;
  const observer=new ResizeObserver(draw);observer.observe(host);window.addEventListener('resize',draw);return()=>{alive=false;observer.disconnect();window.removeEventListener('resize',draw);img.onload=null;img.onerror=null};
 },[src,density]);
