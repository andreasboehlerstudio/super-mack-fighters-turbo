...
import {activeMove,createMatch,step,aiInput,createBrain,TICK,FLOOR,type Match,type Input} from './combat.ts';
...
export interface BattleConfig {p1:FighterId;p2:FighterId;stationId:string;cpu:boolean;difficulty:number;parry?:boolean;turbo?:number;partners?:[FighterId|null,FighterId|null];costumes?:[Costume,Costume];}
...
export function mountBattle(parent:HTMLElement,config:BattleConfig,audio:ArcadeAudio,onState:(m:Match)=>void,onComplete:(winner:number)=>void,onPause:()=>void):BattleHandle {
 let costumesReady=false,paused=false,match=createMatch(config.p1,config.p2,19,{parry:config.parry,turbo:config.turbo,partners:config.partners,stageId:config.stationId}),accumulator=0,ended=false;const controls=new Controls(()=>{if(!paused){paused=true;audio.pause(true);onPause();}}),brain=createBrain();
 class Arena extends Phaser.Scene {
  ...
  preload(){this.load.spritesheet('arena-crowd','/assets/arena-crowd.png',{frameWidth:256,frameHeight:256});this.load.image('arena',ARENA_ART[config.stationId]?.url??`/assets/arena-${this.arena}.png`);for(const id of new Set([config.p1,config.p2,...(config.partners??[]).filter((id):id is FighterId=>id!==null)])){if(!id)continue;if(!this.textures.exists(id))this.load.spritesheet(id,`/assets/${id}.png`,{frameWidth:256,frameHeight:256});this.load.spritesheet(id+'-walk',`/assets/walk/${id}.png`,{frameWidth:256,frameHeight:256});}}
  create(){const jobs:Promise<void>[]=[];for(const side of [0,1]){const costume=config.costumes?.[side]??'classic';if(costume==='classic')continue;for(const id of match.teams[side])for(const kind of ['action','walk'] as const){jobs.push(loadCostumeSheet(id,costume,kind).then(sheet=>{if(!this.sys.isActive())return;const key='costume-'+side+'-'+id+(kind==='walk'?'-walk':'');if(!this.textures.exists(key))this.textures.addSpriteSheet(key,this.textures.addCanvas(key,sheet)!,{frameWidth:256,frameHeight:256})}))}}void Promise.all(jobs).then(()=>{costumesReady=true;onState(match)}).catch(error=>{console.error(error);costumesReady=true;onState(match)});this.back=this.add.image(480,270,'arena');const source=this.textures.get('arena').getSourceImage() as HTMLImageElement;const photo=!!ARENA_ART[config.stationId];if(photo){this.textures.get('arena').setFilter(Phaser.Textures.FilterMode.NEAREST);const scale=Math.max(980/source.width,552/source.height);this.back.setDisplaySize(source.width*scale,source.height*scale);this.add.rectangle(480,508,960,65,0x162337,.07);}else{this.back.setDisplaySize(1060,600);this.textures.get('arena').add('ground',0,0,Math.floor(source.height*.79),source.width,Math.floor(source.height*.21));this.floor=this.add.image(480,483,'arena','ground').setDisplaySize(1120,125);}this.life=new ArenaLife(this,config.stationId);this.shadows=match.actors.map(a=>this.add.ellipse(a.x,FLOOR+3,112,20,0x151728,.35));
   this.sprites=match.actors.map(a=>this.add.sprite(a.x,FLOOR+a.y,a.id,0).setOrigin(.5,246/256).setDisplaySize(274,274));this.fx=this.add.graphics();this.banner=this.add.text(480,190,'',{fontFamily:'Pixelify Sans',fontSize:'42px',fontStyle:'bold',color:'#fff2ca',stroke:'#151528',strokeThickness:9,align:'center'}).setOrigin(.5).setDepth(30);audio.setMode(config.p2==='graumacher'?'boss':'fight');audio.sound('select');
  }
  update(_time:number,delta:number){if(paused||!costumesReady)return;accumulator+=Math.min(delta/1000,.1)*match.rules.turbo;while(accumulator>=TICK){accumulator-=TICK;const p1=controls.read(0),p2=config.cpu?aiInput(match,1,config.difficulty,brain):controls.read(1);step(match,[p1,p2]);for(const event of match.events){audio.sound(event.type);if(event.type==='hit'||event.type==='ko'||event.type==='win')this.life.cheer(match.tick);if(event.type==='hit'){this.cameras.main.shake(85,.003);this.burst(event.x??480,event.y??340,0xffdd65);}if(event.type==='ultra'){this.cameras.main.flash(160,180,200,255);this.callout(event.label??'ULTRA!',event.player??0);this.life.cheer(match.tick);}
   ...
   const center=(match.actors[0].x+match.actors[1].x)/2-480;this.back.x=480-center*.055;if(this.floor)this.floor.x=480-center*.14;this.fx.clear();this.life.draw(match.tick,center*.055);const stage=stagePhysics(config.stationId,match.tick),ground=FLOOR+stage.floorOffset;
   ...
   match.actors.forEach((a,i)=>{const s=this.sprites[i],age=a.age;let frame=0;if(a.action==='idle')frame=Math.floor(age/16)%2;if(a.action==='walk')frame=2+Math.floor(age/8)%2;if(a.action==='jump')frame=4;if(a.action==='crouch')frame=5;if(a.action==='punch')frame=age<8?6:7;if(a.action==='kick')frame=age<14?8:9;if(a.action==='block')frame=10;if(a.action==='hit')frame=11;if(a.action==='special')frame=age<activeMove(a).startup?12:13;if(a.action==='ultra')frame=age<28?12:13;if(a.action==='parry')frame=10;if(a.action==='tag')frame=0;if(a.action==='defeat')frame=14;if(a.action==='victory')frame=15;
    ...
    const costumeKey='costume-'+i+'-'+a.id+(isWalking?'-walk':'');const texture=this.textures.exists(costumeKey)?costumeKey:isWalking?a.id+'-walk':a.id;s.setTexture(texture,frame).setPosition(Math.round(a.x),Math.round(ground+a.y)).setFlipX(a.face<0).setDisplaySize(274*(1-breath*.4),274*(1+breath));s.alpha=a.action==='hit'&&Math.floor(match.tick/3)%2===0?.55:1;this.shadows[i].setPosition(a.x,ground+5).setScale(1+a.y/500).setAlpha(.35+a.y/700);
    if(a.action==='special'&&activeMove(a).kind==='dash'&&age>18&&age<34&&match.tick%4===0){const echo=this.add.sprite(s.x-a.face*14,s.y,texture,frame).setOrigin(.5,246/256).setDisplaySize(274,274).setFlipX(a.face<0).setTint(Phaser.Display.Color.HexStringToColor(fighter(a.id).color).color).setAlpha(.35).setDepth(1);this.tweens.add({targets:echo,alpha:0,duration:180,onComplete:()=>echo.destroy()});}
