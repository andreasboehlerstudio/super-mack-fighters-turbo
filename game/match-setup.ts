...
/** Draw only playable opponents, avoiding the lead fighter and immediate repeats. */
export function randomOpponent(player:FighterId,previous?:FighterId,random:()=>number=Math.random):FighterId {
 const pool=HEROES.filter(h=>h.id!==player&&h.id!==previous);
...
export function makeBattleConfig(options:{runId?:string;mode:GameMode;p1:FighterId;p2:FighterId;arena:string;route:string[];index:number;difficulty:number;rules:GameRules;pads:PlayerPads}):BattleConfig {
 ...
 return {boss,training:mode==='training',p1,p2:opponent,stationId:current,cpu:mode!=='versus',difficulty:Math.min(.92,difficulty+(tour?(index/Math.max(1,route.length-1))*.36+(boss==='midboss'?.08:boss==='finalboss'?.16:0):0)),parry:rules.parry,turbo:rules.turbo,partners:rules.tag?[partner1,partner2]:[null,null],costumes:['classic','classic'],playerPads:[pads[0],mode==='versus'?pads[1]:null]};
}
