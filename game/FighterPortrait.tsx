...
export function FighterPortrait({id,className=''}:{id:FighterId;className?:string}){
 ...
 return <img className={`fighter-portrait ${className}`} src={`/assets/portraits/${id}.png?v=arcade-4`} alt={`Pixelporträt von ${fighter(id).name}`} draggable={false} onError={()=>setFailed(id)}/>;
}
