...
export function instrumentSamples(kind:Instrument):Float32Array {
 const drum=isDrum(kind),length=drum?Math.floor(SAMPLE_RATE*(kind==='hat'?.06:.24)):PERIOD,data=new Float32Array(length);let seed=371,previous=0;
 for(let i=0;i<length;i++){
  ...
  if(drum){const t=i/SAMPLE_RATE;seed=(Math.imul(seed,1664525)+1013904223)>>>0;const noise=seed/4294967296*2-1,high=noise-previous;previous=noise;
   sample=kind==='kick'?Math.sin(2*Math.PI*(44*t+70*.035*(1-Math.exp(-t/.035))))*Math.exp(-t*22):kind==='snare'?(noise*.65+Math.sin(2*Math.PI*175*t)*.25)*Math.exp(-t*24):high*.45*Math.exp(-t*65);
...
export class SnesMusic {
 private context:BaseAudioContext;private output:GainNode;private bank=new Map<Instrument,AudioBuffer>();private voices=new Set<Voice>();
 ...
 setScore(score:MusicScore){if(!(score.duration>0)||!score.notes.length)throw new Error('Leere Musiksequenz');this.stopVoices();this.score=score;this.offset=0;this.cursor=0;this.origin=this.context.currentTime+.06;}
 ...
 private buffer(kind:Instrument){let buffer=this.bank.get(kind);if(!buffer){const data=instrumentSamples(kind);buffer=this.context.createBuffer(1,data.length,SAMPLE_RATE);buffer.getChannelData(0).set(data);this.bank.set(kind,buffer)}return buffer;}
 play(note:MusicNote,time:number){
  ...
  const duration=Math.min(30,Math.max(.02,note.duration)),attack=Math.min(duration*.3,note.instrument==='strings'?.045:.009),level=Math.min(1,note.velocity)*(note.instrument==='bass'?.55:drum?.48:.3);
