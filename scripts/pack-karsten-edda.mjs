...
const jobs=[];for(const id of ids){const folder=path.join(repo,'art-source/characters',id);const records=JSON.parse(await readFile(path.join(folder,'prompts.json'),'utf8'));jobs.push(...records.map(j=>({...j,source:path.join(folder,(j.sourceClip??j.clip)+'.png')})));}
...
async function decodeSheet(j){
 ...
 for(let p=0;p<data.length;p+=4){const key=j.chromaKey&&(j.chromaColor==='magenta'?data[p]>110&&data[p+2]>110&&data[p]>data[p+1]+35&&data[p+2]>data[p+1]+35:data[p+1]>110&&data[p+1]>data[p]+35&&data[p+1]>data[p+2]+35);data[p+3]=!key&&data[p+3]>128?255:0;if(!data[p+3])data.fill(0,p,p+3);}
...
for(const id of ids){
 ...
 const cell=id==='janhulk'?384:256,baseline=cell-10,center=cell/2,height=id==='janhulk'?294:196,headBand=id==='edda'?.34:id==='scholz'?.12:id==='robbemond'?.10:.16,idleScale=height/median(sources.idle.map(f=>f.b.height));
 ...
 for(const [clip,src] of Object.entries(sources)){
  ...
  for(let i=0;i<8;i++){
   ...
   prepared.push({f,w,h,data,b,head,x:Math.round(center-head.left-head.width/2),y:isAir||reactions&&i===0?baseline-height+1-b.top:baseline-b.bottom});
  }
  const shiftX=Math.min(0,(cell-3)-Math.max(...prepared.map(p=>p.x+p.b.left+p.b.width-1)));
  const minY=Math.min(...prepared.map(p=>p.y+p.b.top)),maxY=Math.max(...prepared.map(p=>p.y+p.b.bottom)),shiftY=minY<2?2-minY:Math.min(0,(cell-3)-maxY);
  const frames=[],ms=[];
  for(let i=0;i<8;i++){
   const p=prepared[i],desiredX=p.x+shiftX,x=Math.max(2-p.b.left,Math.min((cell-3)-(p.b.left+p.b.width-1),desiredX)),y=p.y+shiftY,out=Buffer.alloc(cell*cell*4);
   let clipped=0;for(let sy=0;sy<p.h;sy++)for(let sx=0;sx<p.w;sx++){const at=(sy*p.w+sx)*4;if(p.data[at+3]){const dx=x+sx,dy=y+sy;if(dx<2||dx>(cell-3)||dy<2||dy>(cell-3)){clipped++;continue;}p.data.copy(out,(dy*cell+dx)*4,at,at+4);}}
   if(clipped)throw Error(`${id}/${clip}/${i}: ${clipped} clipped pixels`);
   // Optional fixed raster for newly authored pixel art. Keep the common sole at y=baseline.
   if(all.find(j=>j.clip===clip)?.pixelStep===2){
    const raster=Buffer.alloc(out.length);
    for(let gy=1;gy<(cell-1);gy+=2)for(let gx=0;gx<cell;gx+=2){
     const sample=((gy+1)*cell+gx+1)*4;
     for(let dy=0;dy<2;dy++)for(let dx=0;dx<2;dx++)out.copy(raster,((gy+dy)*cell+gx+dx)*4,sample,sample+4);
    }
    raster.copy(out);
   }
   const b=bounds(out,cell,cell),headHeight=Math.round(height*(id==='edda'?.43:.30));
   const fullHead=bounds(out,cell,cell,b.top,b.top+headHeight);
   ...
   ms.push({...b,frame:i,origin:{x:center+shiftX+x-desiredX,y:baseline+shiftY},headWidth:p.head.width,headX:x+p.head.left+p.head.width/2,scale,nativeBodyHeight:p.f.b.height,nativeSpan:Math.max(p.f.b.height,p.f.b.width),hash:createHash('sha256').update(out).digest('hex'),anchor});
   frames.push(await sharp(out,{raw:{width:cell,height:cell,channels:4}}).png().toBuffer());
  }
  ...
  await sharp({create:{width:cell*4,height:cell*2,channels:4,background:'#00000000'}}).composite(frames.map((input,i)=>({input,left:i%4*cell,top:Math.floor(i/4)*cell}))).png().toFile(path.join(dir,'normalized',id+'-'+clip+'.png'));
  const row={id,clip,scale,idleHeight:height,groundRatio:height/cell,frames:ms.map(({anchor,...f})=>f)};report.push(row);if(!['idle','walk'].includes(clip))combatAudit.sheets.push(row);
 }
 origins[id]={};
 ...
 for(const [kind,frames,columns,rows]of [['action',action,4,4],['walk',framesByClip.walk,4,2],['motion',motion,8,6]]){
  const target=path.join(assets,kind==='action'?'':kind,id+'.png');await sharp({create:{width:columns*cell,height:rows*cell,channels:4,background:'#00000000'}}).composite(frames.map((input,i)=>({input,left:i%columns*cell,top:Math.floor(i/columns)*cell}))).png({compressionLevel:9}).toFile(target);
 }
 ...
 for(const [clip,spec]of Object.entries(ANIMATION_SHEETS)){
  ...
  const width=Math.min(4,frames.length)*cell,height=Math.ceil(frames.length/4)*cell;
  const raw=await sharp({create:{width,height,channels:4,background:'#00000000'}}).composite(frames.map((input,i)=>({input,left:i%4*cell,top:Math.floor(i/4)*cell}))).raw().toBuffer();
 ...
 idleAudit.push({id,frames:8,bodyHeight:height,maxBreathingHeightDifference:Math.max(...measurements.idle.map(f=>f.height))-Math.min(...measurements.idle.map(f=>f.height)),baseline:baseline,originX:center});
 ...
 const montage=[];for(const [row,clip]of ['idle','walk','punch','kick','airpunch','airkick','reactions'].entries())for(let i=0;i<8;i++)montage.push({input:framesByClip[clip][i],left:i*cell,top:row*cell});
 await sharp({create:{width:cell*8,height:cell*7,channels:4,background:'#182337'}}).composite(montage).png().toFile(path.join(dir,id+'-review.png'));
}
