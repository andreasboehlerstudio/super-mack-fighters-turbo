...
const source=process.argv[2]?path.resolve(process.argv[2]):path.join(root,'public/assets/atlas/park-1-detail.webp');
...
if(source!==path.join(target,'park-1-detail.webp'))await sharp(source).webp({lossless:true,effort:6}).toFile(path.join(target,'park-1-detail.webp'));
await sharp(source).resize(1484,1060,{kernel:'nearest'}).webp({lossless:true,effort:6}).toFile(path.join(target,'park-1-overview.webp'));
const tiles=[];
for(let y=0;y<meta.height;y+=256)for(let x=0;x<meta.width;x+=256){
 const width=Math.min(256,meta.width-x),height=Math.min(256,meta.height-y),file=`${x/256}-${y/256}.webp`;
 await sharp(source).extract({left:x,top:y,width,height}).webp({lossless:true,effort:6}).toFile(path.join(target,'detail',file));
